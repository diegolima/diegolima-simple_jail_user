...
class simple_jail_user (
	...
	define createUser (
		...
		user { "${name}":
			...
			managehome       => true,
			home             => "${home_dir}/${name}",
			password         => generate($simple_jail_user::params::bash_bin, '-c', "echo ${password} | ${simple_jail_user::params::openssl} ${simple_jail_user::params::passwd} -1 -stdin | ${simple_jail_user::params::tr} -d '\n'"),
